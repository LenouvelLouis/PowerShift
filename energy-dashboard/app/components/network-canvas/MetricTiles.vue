  ...
  <div class="flex flex-col gap-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
      ...
      <div class="bg-[#0F172A] border border-[#1E293B] rounded-2xl p-4 flex flex-col gap-1.5">
        ...
        <p class="text-2xl font-bold font-mono" :class="balanceColor">
          {{ fmt(result.balance_mwh) }}
        </p>
        <p class="text-[11px] text-gray-600">MWh</p>
      </div>
      ...
      <!-- Convergence -->
      <div class="bg-[#0F172A] border border-[#1E293B] rounded-2xl p-4 flex flex-col gap-1.5">
        <div class="flex items-center gap-1.5" :class="convergence?.all_converged ? 'text-emerald-500' : 'text-amber-500'">
          <UIcon
        ...
        <p class="text-2xl font-bold font-mono" :class="convergence?.all_converged ? 'text-emerald-400' : 'text-amber-400'">
          {{ convergence ? `${convergence.converged_count}/${convergence.total_snapshots}` : '—' }}
        </p>
        <p class="text-[11px] text-gray-600">snapshots</p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <!-- Capacity factors -->
      <div v-if="capacityFactors.length" class="bg-[#0F172A] border border-[#1E293B] rounded-2xl p-5">
          ...
          <div v-for="{ name, cf, color } in capacityFactors" :key="name">
            <div class="flex items-center justify-between mb-1.5">
              <div class="flex items-center gap-1.5">
                <UIcon :name="generatorIcon(name)" class="w-3 h-3 text-gray-500 shrink-0" />
                <span class="text-xs text-gray-400 truncate max-w-[160px]" :title="name">{{ name }}</span>
              </div>
      ...
      <div class="bg-[#0F172A] border border-[#1E293B] rounded-2xl p-5">
        ...
        <div class="space-y-2.5 text-sm">
          ...
          <div v-if="result.name" class="flex justify-between items-center">
            <span class="text-gray-500">Name</span>
            <span class="text-gray-300 text-xs truncate max-w-40">{{ result.name }}</span>
          </div>
          <div v-if="violations?.overloads?.length" class="flex justify-between items-center">
            <span class="text-gray-500">Overloaded lines</span>
            <span class="text-red-400 font-mono text-xs">{{ violations.overloads.length }}</span>
          </div>
          <div v-if="violations?.overvoltages?.length" class="flex justify-between items-center">
